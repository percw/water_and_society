...
4. Methodology
The research is structured in two phases: Phase 1 uses lightweight, pre-computed datasets to establish the core empirical signal. Phase 2 (future work) applies computationally intensive NLP to validate and deepen the Phase 1 findings.

PHASE 1 — Lightweight Empirical Analysis (Ngram + GDP)

Phase 1.1: Ngram Frequency Trajectories
Objective: To visually and statistically establish whether water-related industrial vocabulary rose before fossil-fuel vocabulary in the published record.

Process: Using the Google Books Ngram Corpus (English, 1700–1900), we extract raw word frequency timeseries for target terms: water, canal, mill, pump (hydro-industrial) and steam, coal, engine (fossil-industrial). We plot their trajectories to identify which vocabulary cluster achieved prominence first.

Phase 1.2: Dictionary-Based Classification (Agrarian vs. Industrial Water)
Objective: To quantify the conceptual transition of water from a natural/agrarian context to an industrial/engineering context.

Process: We define two manually curated dictionaries — Agrarian Water (flood, rain, divine, river, harvest, holy) and Industrial Water (canal, pump, mill, engine, factory, power, engineer, machine). Using Ngram frequency data, we compute the ratio of "Industrial Water" to "Agrarian Water" over time to track the commodification of water.

Phase 1.3: Macroeconomic Overlay and Granger Causality Testing
Objective: To link the linguistic frequency shifts from Phase 1.1 and 1.2 to actual economic growth.

Process: We overlay the Ngram frequency trajectories and the Industrial/Agrarian ratio against historical GDP per capita from the Maddison Project Database (2023) for Britain, China, and India. We conduct Granger causality tests to determine if the hydro-linguistic shift preceded the economic "hockey stick" of the British Industrial Revolution.

PHASE 2 — Deep NLP Analysis (Future Work, Computationally Intensive)

Phase 2.1: Topic Modeling (Latent Dirichlet Allocation - LDA)
...
Phase 2.2: Diachronic Word Embeddings (Temporal Word2Vec)
...
Phase 2.3: Full Corpus Comparative Analysis
Objective: To contrast the British linguistic trajectories against equivalent HathiTrust corpora for China and India, demonstrating the absence of the hydro-social transition in non-industrializing regions.
...
6. Proposed Timeline
Month 1: Data Acquisition. Downloading Google Books Ngram data and structuring the Maddison GDP data. Running Phase 1 frequency and dictionary analyses.

Month 2: Phase 1 Completion. Completing the macroeconomic overlay, Granger causality tests, and writing up Phase 1 findings.

Month 3: HathiTrust Acquisition. Querying the HathiTrust API, downloading Extracted Features for Phase 2.

Month 4: Word Embeddings. Training Temporal Word2Vec models to calculate cosine similarities across time slices for "water," "steam," and "coal."
