...
### 4.7 Double/Debiased Machine Learning Robustness

A known limitation of Two-Way Fixed Effects (TWFE) DiD is its reliance on a linearity assumption: that the treatment effect is constant across units and time, and that confounders relate to the outcome in a parametric fashion. Violations of this assumption — through non-linear time trends, heterogeneous treatment timing, or unmodelled interaction effects — can bias the canonical TWFE estimator even when the parallel trends assumption holds in expectation (Callaway & Sant'Anna 2021; Sun & Abraham 2021).

To counter these limitations, we deployed a **Causal Forest** algorithm within the **Double/Debiased Machine Learning (DML)** framework of Chernozhukov et al. (2018), providing a non-parametric robustness check against functional form misspecification. Rather than assuming linearity between covariates and outcomes, the DML approach uses machine learning to partial out confounders in a first stage, then recovers the causal parameter from the residuals via a clean second-stage regression.

**Algorithm.** The DML procedure proceeds in three steps. First, we use $k$-fold cross-fitting to train two Random Forest models (Wager & Athey 2018): one predicting GDP per capita from year and country indicators ($\hat{Y} = \mathbb{E}[Y \mid X]$), and a second predicting the DiD interaction term from the same covariate set ($\hat{D} = \mathbb{E}[D \mid X]$). Cross-fitting (with $k = 5$ folds) ensures that predictions are made out-of-sample, avoiding overfitting bias that would otherwise contaminate the second-stage estimate. In the second step, we compute the partialled-out residuals: $\tilde{Y} = Y - \hat{Y}$ and $\tilde{D} = D - \hat{D}$. In the third step, we regress $\tilde{Y}$ on $\tilde{D}$ via OLS with HC3 heteroskedasticity-robust standard errors, yielding $\hat{\beta}_{\text{DML}}$ — the treatment effect estimate after non-parametrically removing the confounding influence of time trends and country-level baselines.

**Heterogeneous Treatment Effects.** To replicate the core functionality of a Causal Forest — estimating *heterogeneous* treatment effects (HTE) across subgroups — we further disaggregate the DML residual regression by decade. Regressing $\tilde{Y}$ on $\tilde{D}$ within each ten-year window produces decade-specific HTE estimates ($\hat{\beta}_{\text{HTE}}$) that surface temporal variation in the treatment effect without imposing any parametric structure on how the effect evolves.

**Results.** The DML treatment effect estimate $\hat{\beta}_{\text{DML}}$ is consistent in sign and order of magnitude with the TWFE benchmark ($\hat{\beta}_3 = 1{,}250.9$, HAC $p = 0.042$), confirming that the core finding is not an artefact of parametric assumptions. The decade-level HTE analysis reveals that treatment effects are negligible in the pre-1761 period (consistent with the parallel trends validation in §4.5) and rise persistently following the Bridgewater shock, with the largest magnitudes observed during the height of the canal era (1770–1810) before stabilising in the steam transition. This pattern corroborates the event study in **Figure 2** without requiring linearity or common treatment timing, and is visualised in **Figure 4**.

<div align="center">
  <img src="../../data/did_causal_forest_dml.png" alt="Figure 4: Causal Forest DML Robustness" width="800">
  <br>
  <em><strong>Figure 4: Causal Forest DML Robustness Check.</strong> <em>Panel A</em> plots the second-stage DML regression: outcome residuals ($\tilde{Y}$) against treatment residuals ($\tilde{D}$) after a Random Forest first stage non-parametrically removes the influence of year trends and country fixed effects. The positive slope ($\hat{\beta}_{\text{DML}}$) confirms the core treatment effect survives partialling out without linearity assumptions. <em>Panel B</em> shows decade-level heterogeneous treatment effect estimates ($\hat{\beta}_{\text{HTE}}$) derived from the same DML residuals — a Causal Forest analog — demonstrating that effects emerge after 1761 and peak during the canal era, consistent with the TWFE event study.</em>
</div>
