...
about sequence, and only Britain can test it on annual data. Using
...
At the Maddison benchmarks the Dutch, Belgian, Chinese and Indian cases
follow the same rule: water without coal, or coal without engineered
...
sequence can be tested only where annual series exist: for output by
sector, population, canal mileage and installed power from 1700, Britain
is the one case where they do. The comparative cases can then be read
through its result.
...
run the sequence compressed. Annual series of the British kind do not
exist elsewhere, so the check is at the Maddison benchmark years, where
population is measured rather than interpolated, read alongside what the
national literatures record about fuel and water. Table 6 gives growth
between 1700 and 1820, before steam supplied a third of British power,
and between 1820 and 1870.
...
ones. Population in the Maddison database is interpolated between 1700
and 1820, so cross-country regressions on total output over that period
are not legitimate and we have not run them; Table 6 is descriptive, and
its Maddison row for Britain is the United Kingdom including Ireland.
The trekvaart mileage, the Belgian canal dates and the location of
Chinese and Indian coal are taken from the works cited, and the 1700
Chinese benchmark is contested. The war-drawdown result is a property of
the controls and so is robust to their choice, but it means that
cross-country per-capita evidence for a British take-off before 1815 is,
in our reading, unrecoverable from the Maddison series with any
